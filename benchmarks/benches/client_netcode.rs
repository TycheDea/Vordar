...
fn bench_apply_states(c: &mut Criterion) {
    ...
    for a in [64usize, 200] {
        ...
        let enters: Vec<EntityState> = spots
            .iter()
            .enumerate()
            .map(|(i, &pos)| EntityState { id: i as u64 + 1, prefab: "bolt".into(), pos })
            .collect();
        ...
        let states: Vec<EntityPos> = spots
            .iter()
            .enumerate()
            .map(|(i, &pos)| EntityPos { id: i as u64 + 1, pos: pos + Vec3::X })
            .collect();
        ...
        b.iter_custom(|iters| {
            ...
            for _ in 0..iters {
                let enters: Vec<EntityState> = spots
                    .iter()
                    .enumerate()
                    .map(|(i, &pos)| EntityState { id: next_id + i as u64, prefab: "bolt".into(), pos })
                    .collect();
                let t = Instant::now();
                seam::apply_snapshot(&mut world, &mut resources, 0, enters, Vec::new(), Vec::new());
                total += t.elapsed();
